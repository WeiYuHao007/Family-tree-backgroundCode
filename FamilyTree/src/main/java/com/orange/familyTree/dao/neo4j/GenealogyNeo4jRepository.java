...
public interface GenealogyNeo4jRepository extends Neo4jRepository<GenealogyNeo4j, Long>{
	...
	@Query("MATCH (u:UserMySQL) \n" +
			"WHERE u.id = {adminId} \n" +
			"CREATE (g:GenealogyMySQL{name: {genealogyName},genealogyId: {genealogyId}}) \n" +
	...
	// 增设图谱管理员（Neo4j）
	@Query("MATCH (g:GenealogyMySQL)<-[r:FOCUS_ON{show: true, admin: false}]-(u:UserMySQL) \n" +
	...
	// 转让图谱管理员（Neo4j）
	@Query("MATCH (u1:UserMySQL)-[r1:FOCUS_ON]->(g:GenealogyMySQL)<-[r2:FOCUS_ON]-(u2:UserMySQL) \n" +
