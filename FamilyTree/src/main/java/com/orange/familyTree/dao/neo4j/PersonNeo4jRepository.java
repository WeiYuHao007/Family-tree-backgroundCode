...
public interface PersonNeo4jRepository extends Neo4jRepository<Person, Long>{
	...
	// 查询指定节点的信息
	@Query("MATCH (g:GenealogyMySQL)-[:OWNS]->(p:Person)" +
	...
	@Query("MATCH (g:GenealogyMySQL)-[:OWNS]->(p1:Person)\n" +
	...
	// 查询指定节点的儿子（Neo4j）
	@Query("MATCH (g:GenealogyMySQL)-[:OWNS]->(p1:Person)<-[:IS_SON]-(p2:Person) \r\n" +
	...
	// 查询两个相邻节点间的关系（Neo4j）
	@Query("MATCH (p1:Person)<-[:OWNS]-(g:GenealogyMySQL)-[:OWNS]->(p2:Person), (p1)-[r]->(p2) \r\n" +
	...
	// 创建关系（父母 <-儿子）（Neo4j）
	@Query("MATCH (p1:Person)<-[:OWNS]-(g:GenealogyMySQL)-[:OWNS]->(p2:Person) \n" +
